...
export default function ArchivePage() {
  ...
  useEffect(() => {
    ...
    // Search filter
    if (searchQuery) {
      filtered = filtered.filter((item) => item.planetName.toLowerCase().includes(searchQuery.toLowerCase()))
    }

    // Status filter
    if (statusFilter !== "all") {
      filtered = filtered.filter((item) => item.prediction === statusFilter)
    }

    // Star type filter
    if (starTypeFilter !== "all") {
      filtered = filtered.filter((item) => item.inputs?.starType === starTypeFilter)
    }

    setFilteredArchive(filtered)
  }, [searchQuery, statusFilter, starTypeFilter, archive])
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <h1 className="text-4xl md:text-5xl font-bold">
                  Classification <span className="text-primary">Archive</span>
                </h1>
              ...
              <div className="grid md:grid-cols-4 gap-4">
                ...
                <Select value={statusFilter} onValueChange={setStatusFilter}>
                  ...
                  <SelectContent className="glass-panel border-primary/30">
                    ...
                    <SelectItem value="Not Habitable">Not Habitable</SelectItem>
                  </SelectContent>
                </Select>

                {/* Star Type Filter */}
                <Select value={starTypeFilter} onValueChange={setStarTypeFilter}>
                  <SelectTrigger className="glass-panel border-primary/30">
                    <Calendar className="w-4 h-4 mr-2" />
                    <SelectValue placeholder="Star Type" />
                  </SelectTrigger>
                  <SelectContent className="glass-panel border-primary/30">
            ...
            {filteredArchive.length === 0 ? (
              ...
              <div className="space-y-4">
                ...
                {filteredArchive.map((item, index) => {
                  const isHabitable = item.prediction === "Habitable"
                  return (
                    <motion.div
                      key={index}
                      initial={{ y: 20, opacity: 0 }}
                      ...
                      <Card className="glass-panel p-4 hover:border-primary/50 transition-all group">
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
                          ...
                          <div className="col-span-1 md:col-span-2 flex items-center gap-3">
                            ...
                            <div>
                              <p className="font-semibold">{item.planetName}</p>
                              <p className="text-xs text-muted-foreground md:hidden">
                                {item.inputs?.starType || "N/A"} Star
                              </p>
                            </div>
                          </div>
